...
- `book_translation_agents.py`
  CLI workflow. It now imports shared prompt/parsing helpers from
  `translation_core.py`, while keeping CLI-specific model setup, retries, review,
  revision, and artifact writing.

- `document_adapters.py`
...
This enables the dev-only fake translator in `backend_translation.py`. It
...
`backend_api.py` owns the normalization helpers and FastAPI exception handlers.
...
The frontend `formatApiError` in `frontend/app.js` reads this shape and appends
...
`backend_mysql_schema.sql`, and `backend_mysql_store.py` auto-creates the
