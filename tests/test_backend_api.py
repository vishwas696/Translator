...
from translator.api import app as backend_api
from translator.billing.razorpay import RazorpayOrderResult, RazorpayWebhookSignatureError
from translator.storage.local import LocalDocumentStore
...
class BackendApiTests(unittest.TestCase):
    def test_backend_store_defaults_to_mysql(self) -> None:
        ...
        try:
            with patch("translator.storage.mysql.MySqlDocumentStore", FakeMySqlDocumentStore):
    ...
    def test_razorpay_requires_webhook_before_checkout_order_is_created(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("translator.api.app.razorpay_payments_configured", return_value=False), patch(
                "translator.api.app.razorpay_orders_configured",
                return_value=True,
            ), patch(
                "translator.api.app.razorpay_webhook_configured",
                return_value=False,
            ):
                response = client.post(
                    "/billing/checkout-session",
    ...
    def test_razorpay_checkout_uses_server_package_values(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("translator.api.app.razorpay_payments_configured", return_value=True), patch(
                "translator.api.app.razorpay_checkout_key_id",
                return_value="rzp_test_key",
            ), patch(
                "translator.api.app.create_razorpay_order",
    ...
    def test_razorpay_webhook_completes_payment_idempotently(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with patch("translator.api.app.construct_razorpay_webhook_event", return_value=event):
                    first = client.post(
    ...
    def test_razorpay_webhook_rejects_signature_and_amount_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            backend_api.store = LocalDocumentStore(Path(temp_dir) / "backend")
            client = TestClient(backend_api.app)
            with patch("translator.api.app.construct_razorpay_webhook_event", side_effect=RazorpayWebhookSignatureError()):
    ...
    def test_razorpay_webhook_rejects_untrusted_payment_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("translator.api.app.construct_razorpay_webhook_event", return_value=wrong_owner_event):
                wrong_owner = client.post(
                    "/billing/razorpay/webhook",
                    headers={"X-Razorpay-Signature": "signed"},
                    content=b"{}",
                )
            with patch("translator.api.app.construct_razorpay_webhook_event", return_value=wrong_package_event):
                wrong_package = client.post(
                    "/billing/razorpay/webhook",
                    headers={"X-Razorpay-Signature": "signed"},
                    content=b"{}",
                )
            with patch("translator.api.app.construct_razorpay_webhook_event", return_value=not_captured_event):
    ...
    def test_google_auth_mode_sets_owner_from_verified_token(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                backend_api.store = LocalDocumentStore(Path(temp_dir) / "backend")
                client = TestClient(backend_api.app)
                with patch("translator.api.auth.verify_google_id_token", side_effect=fake_verify):
                    upload_response = client.post(
                        "/documents/upload",
